...
- Browser demo corruption root-cause fix: removed nested paragraph generation in table cells (`<w:p><w:p>...`) by correcting table serialization paths in `services/table-reconciliation.js`.
- Browser demo enhancement: kitchen-sink flow now also applies highlight and inserts a comment, including `comments.xml` artifact wiring (`[Content_Types].xml` + `document.xml.rels`) and validation checks.
